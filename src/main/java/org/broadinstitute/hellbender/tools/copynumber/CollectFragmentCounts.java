...
/**
 * Collects fragment counts at specified intervals.  For each fragment, a count is added to an interval if the
 * interval center is the closest to the fragment center, when compared against all other interval centers for
 * intervals that overlap with the fragment.  The start and end positions of fragments are
...
public final class CollectFragmentCounts extends ReadWalker {
    ...
    public void apply(GATKRead read, ReferenceContext referenceContext, FeatureContext featureContext) {
        ...
        try {
            ...
            final Set<SimpleInterval> overlappingIntervals = intervalOverlapDetector.getOverlaps(fragmentCenter);
            ...
            //determine interval with center closest to fragment center
            final SimpleInterval closestInterval = overlappingIntervals.stream()
                    .min(Comparator.comparing(si -> Math.abs((si.getEnd() - si.getStart()) / 2. - fragmentCenter.getStart())))
                    .get();
            intervalMultiset.add(closestInterval);
